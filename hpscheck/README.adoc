...
[width="80%",cols="^2,10,5",options="header"]
|===
|WAIT EVENT TYPE |Descryption | WAIT EVENT

|Activity 
|서버 프로세스가 유휴 상태. 이 유형은 main processing loop에서 activity를 기다리는 프로세스
|ArchiverMain, AutoVacuumMain, BgWriterHibernate, BgWriterMain, CheckpointerMain, LogicalApplyMain, LogicalLauncherMain, PgStatMain, RecoveryWalStreawm, SysLoggerMain, WalReceiverMain, WalSenderMain, WalWriterMain
